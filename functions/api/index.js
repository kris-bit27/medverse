...
function generateCacheKey(mode, context) {
  const normalized = JSON.stringify({ mode, context });
...
PRAVIDLA: Cituj zdroje. Vrať JSON: {"full_text": "markdown 3000-5000 slov", "confidence": 0.85, "sources": [], "warnings": []}`,
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 4096,
      temperature: 0.3,
      system: systemPrompts[mode] || systemPrompts['topic_generate_fulltext_v2'],
      messages: [{ role: 'user', content: userPrompts[mode] || userPrompts['topic_generate_fulltext_v2'] }]
    });
